...
def denoise(diffuseNet, specularNet, dataloader, device, mode, save_dir, debug=False):
  with torch.no_grad():
    ...
    for data in tqdm(dataloader, leave=False, ncols=70):
      ...
      # visualize
      if not os.path.exists(save_dir + '/test{}'.format(batch_idx+1)):
        os.makedirs(save_dir + '/test{}'.format(batch_idx+1))
      ...
      # writer.add_image('noisy patches e{}'.format(batch_idx+1), inputGrid)
      save_image(inputFinal, save_dir + '/test{}/noisy.png'.format(batch_idx+1))

      outputGrid = torchvision.utils.make_grid(outputFinal)
      # writer.add_image('denoised patches e{}'.format(batch_idx+1), outputGrid)
      save_image(outputFinal, save_dir + '/test{}/denoise.png'.format(batch_idx+1))

      cleanGrid = torchvision.utils.make_grid(Y_final)
      # writer.add_image('clean patches e{}'.format(batch_idx+1), cleanGrid)
      save_image(Y_final, save_dir + '/test{}/clean.png'.format(batch_idx+1))
...
def test_model(diffuseNet, specularNet, device, data_dir, mode, args):
  ...
  dataset = DenoiseDataset(data_dir, 8, 'kpcn', 'test', 4, 'grid',
...
def main():
  ...
  if 'feat' in args.mode:
    diffuseNet = make_senet(args.num_layers, args.input_channels, args.hidden_channels, args.kernel_size, args.mode)
    specularNet = make_senet(args.num_layers, args.input_channels, args.hidden_channels, args.kernel_size, args.mode)
  else:
    diffuseNet = make_net(args)
    specularNet = make_net(args)
